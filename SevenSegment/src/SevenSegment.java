...
public class SevenSegment extends JPanel implements ActionListener
{
    ...
    public void initializeSegments()
    {
        ...
        for (int i = 0; i < 7; i++)
            segments[i] = false;
        segments2[0] = new RoundRectangle2D.Double(3 * width / 12, height / 12, 6 * width / 12,
                height / 12, 10, 10);
        segments2[1] = new RoundRectangle2D.Double(9 * width / 12, 2 * height / 12, width / 12,
                3 * height / 12, 10, 10);
        segments2[2] = new RoundRectangle2D.Double(9 * width / 12, 6 * height / 12, width / 12,
                3 * height / 12, 10, 10);
        segments2[3] = new RoundRectangle2D.Double(3 * width / 12, 9 * height / 12, 6 * width / 12,
                height / 12, 10, 10);
        segments2[4] = new RoundRectangle2D.Double(2 * width / 12, 6 * height / 12, width / 12,
                3 * height / 12, 10, 10);
        segments2[5] = new RoundRectangle2D.Double(2 * width / 12, 2 * height / 12, width / 12,
                3 * height / 12, 10, 10);
        segments2[6] = new RoundRectangle2D.Double(3 * width / 12, 5 * height / 12, 6 * width / 12,
                height / 12, 10, 10);
        repaint();
    }
    ...
    public void paintComponent(Graphics g)
    {
        ...
        segments[1] = (number[1] && number[0]) || (!number[2])
        ...
        segments[5] = number[3] || !number[1] && (number[2] || !number[0])
                || (!number[0] && number[2]);

        segments[6] = number[3] || number[2] != number[1] || !number[0] && number[1];
